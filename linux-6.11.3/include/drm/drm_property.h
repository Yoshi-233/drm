...
struct drm_property {
	 ...
	 * 这是基于 KMS（内核模式设置）的基本对象，提供了属性的上下文或基础信息
	 */
	 ...
	 * 
	 * 	DRM_MODE_PROP_RANGE：表示属性是一个数值范围。
	 *	DRM_MODE_PROP_SIGNED_RANGE：表示属性是一个有符号的数值范围。
	 *	DRM_MODE_PROP_ENUM：表示属性是一个枚举类型，数值对应于字符串名称。
	 *	DRM_MODE_PROP_BITMASK：表示属性是一个位掩码，可同时包含多个枚举值。
	 *	DRM_MODE_PROP_OBJECT：用于链接不同的 KMS 对象，如将帧缓冲区与平面或 CRTC 链接。
	 ...
	 * 这是一个数组，存储属性的限制和数值。属性的含义和解释依赖于 flags 的值。
	 */
	uint64_t *values;
