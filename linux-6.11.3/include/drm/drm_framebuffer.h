...
struct drm_framebuffer {
	 ...
	 * 一个基础的模式设置对象结构体，包含了引用计数，以管理该帧缓冲区的生命周期
	 * 结构体定义 ./linux-6.11.3/include/drm/drm_mode_object.h
	 */
	struct drm_mode_object base;

	/**
	 * @comm: Name of the process allocating the fb, used for fb dumping.
	 * 用于存储分配此帧缓冲区的进程名，以便在需要时对帧缓冲区进行调试、转储等操作
	 */
	char comm[TASK_COMM_LEN];

	/**
	 * @format: framebuffer format information
	 * 指向帧缓冲区的格式信息，例如颜色深度和像素格式，提供了一种描述存储在帧缓冲区中的图像数据方式
	 */
	const struct drm_format_info *format;
	/**
	 * @funcs: framebuffer vfunc table
	 * 指向函数指针表，包含了与帧缓冲区操作相关的回调函数（如创建、销毁帧缓冲区等）
	 */
	const struct drm_framebuffer_funcs *funcs;
	/**
	 * @pitches: Line stride per buffer. For userspace created object this
	 * is copied from drm_mode_fb_cmd2.
	 * 存储每个缓冲区平面的行跨度，用户空间创建的对象会从 drm_mode_fb_cmd2 中复制该值。
	 */
	 ...
	 * 存储实际像素数据在缓冲区中的偏移量，以字节为单位。
	 * 这是根据每个平面的布局进行的设置，需要考虑到硬件的对齐要求。
	 */
	 ...
	 * drm_mode_fb_cmd2.
	 * 表示数据布局的修改器，用于描述缓冲区的排布方式，如倾斜或特殊布局（如压缩）等。
	 */
	uint64_t modifier;
	/**
	 ...
	 * 存储与帧缓冲区相关的标志，例如是否为交错模式或是否使用修改器。
	 */
	int flags;
	/**
	 * @filp_head: Placed on &drm_file.fbs, protected by &drm_file.fbs_lock.
	 * 链接到 drm_file 结构中的打开帧缓冲区列表，保护该列表的访问
	 */
	 ...
	 * drm_gem_fb_create().
	 * 存储指向支持当前帧缓冲区的 GEM 对象的指针，每一平面一个。
	 * 这些对象是通过 GEM 来处理的，便于内存管理
	 */
